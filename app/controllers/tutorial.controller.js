...
exports.delete = (req, res) => {
	...
	Tutorial.findByIdAndRemove(id).then(data => {
		...
		res.status(500).send({message: "Counld not delete Tutorial with id=" + id});
	});
};
...
exports.deleteAll = (req, res) => {
  Tutorial.deleteMany({}).then(data => {
		res.send({message: `${data.deleteCount} Tutorials were deleted successfully!`});
	}).catch(err => {
		res.status(500).send({message: err.message || "Somne error occurred while removing all tutorials."});
	});
};
